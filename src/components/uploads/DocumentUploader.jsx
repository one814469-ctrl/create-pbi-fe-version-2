...
const DocumentUploader = ({ onUpdateApplication }) => {
    ...
    <div className="document-uploader">
      ...
      {uploadedFiles.length > 0 && (
        <div className="mt-4">
          ...
          <p className="success-message mt-2"><em>Acceptance Criteria: "Given my application requires document uploads, When I select and upload valid files, Then each file is successfully attached to my application" (Mocked)</em></p>
        </div>
      )}
      {uploadMessage.includes('exceeds size limits') && <p className="error-message mt-2"><em>Edge Case: "Given I upload a file that exceeds size limits, When I attempt to upload, Then I see an error and the file is rejected" (Mocked)</em></p>}
      {uploadMessage.includes('Unsupported file format') && <p className="error-message mt-2"><em>Negative Case: "Given I upload a file with an unsupported format, When I attempt upload, Then I receive a format error and the upload is blocked" (Mocked)</em></p>}
    </div>
  );
};
