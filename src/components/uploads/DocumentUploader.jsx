...
const DocumentUploader = ({ onUpdateApplication, task }) => {
  ...
  return (
    <div className="document-uploader">
      ...
      {uploadMessage.includes('exceeds size limits') && task && task.acceptance_criteria[1] && (
        <p className="error-message mt-2">
          <em>Acceptance Criteria (Edge): {task.acceptance_criteria[1]} (Mocked: Error and file rejected)</em>
        </p>
      )}
      {uploadMessage.includes('Unsupported file format') && task && task.acceptance_criteria[2] && (
        <p className="error-message mt-2">
