...
const ApplicationStatusTracker = ({ applicationData }) => {
  const mockStatuses = [
  ...
  if (!applicationData || applicationData.status === 'empty') {
    return (
      <div className="status-tracker card">
        ...
        <p><em>Edge Case: "Given I have no submitted applications, When I visit the Application Status page, Then I see an empty-state card and a prompt to start a new application" (Mocked)</em></p>
  ...
  return (
    <div className="status-tracker card">
      ...
      <p className="mt-4">
        <em>Acceptance Criteria: "Given I am logged in and have an active loan application, When I go to the Application Status page, Then I see the current status and timeline of my application" (Mocked)</em>
      </p>
    </div>
  );
};
