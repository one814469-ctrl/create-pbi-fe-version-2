...
const FormTask = ({ task }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.loanAmount || isNaN(formData.loanAmount) || parseFloat(formData.loanAmount) <= 0) {
      ...
      if (formData.loanType === 'personal' && (parseFloat(formData.loanAmount) < minAmount || parseFloat(formData.loanAmount) > maxPersonalAmount)) {
        newErrors.loanAmount = `Personal loan amount must be between $${minAmount.toLocaleString()} and $${maxPersonalAmount.toLocaleString()}.`; isValid = false;
      } else if (formData.loanType === 'home' && (parseFloat(formData.loanAmount) < minAmount || parseFloat(formData.loanAmount) > maxHomeAmount)) {
        newErrors.loanAmount = `Home loan amount must be between $${minAmount.toLocaleString()} and $${maxHomeAmount.toLocaleString()}.`; isValid = false;
      }
    }
