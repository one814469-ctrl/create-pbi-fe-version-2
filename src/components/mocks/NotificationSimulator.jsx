...
const NotificationSimulator = ({ notificationType, onUpdateApplication, task }) => {
  ...
  const sendNotification = (type) => {
    ...
    setTimeout(() => {
      let message = '';
      let resultType = 'info';
      let notificationDelivered = Math.random() > 0.1; // 90% chance of delivery success

      if (!notificationDelivered && type !== 'internal_reminder') { // Internal reminder has its own edge case
      ...
      switch (type) {
        ...
        case 'internal_reminder':
          ...
          if (hasPending) {
            ...
            resultType = 'success'; // Treat as success that no unnecessary reminder was sent
          }
          onUpdateApplication && onUpdateApplication(prev => ({
            ...prev,
    ...
    <div className="notification-simulator">
      ...
      {lastNotification && (
        <div className={`mock-api-response mt-4 ${lastNotification.type}`}>
          ...
          {notificationType === 'application_status' && lastNotification.type === 'error' && (
            <p className="error-message mt-2">
