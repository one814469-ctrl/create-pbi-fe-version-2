...
const LoanApplicationForm = ({ displayMessage }) => {
  ...
  if (!isAuthenticated) {
    displayMessage('destructive', 'Authentication Required', 'You must be logged in to submit a loan application.');
    return (
  ...
  if (user?.role !== 'customer') {
    displayMessage('destructive', 'Access Denied', `Only customers can submit loan applications. Your role is ${user?.role}.`);
    return (
      <Card className="w-full max-w-lg mx-auto my-8">
        <CardHeader>
          <CardTitle className="text-destructive">Access Denied</CardTitle>
