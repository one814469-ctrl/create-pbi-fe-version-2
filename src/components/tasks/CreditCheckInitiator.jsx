...
const CreditCheckInitiator = ({ displayMessage }) => {
  ...
  if (!isAuthenticated || (user?.role !== 'officer' && user?.role !== 'underwriter')) {
    displayMessage('destructive', 'Access Denied', 'You need loan officer/underwriter permissions to initiate credit checks.');
    return (
        ...
        {creditCheckResult && (
          <div className="space-y-2">
            ...
            {creditCheckResult.error ? (
              ...
              <>
                <p>Credit Score: <strong className={cn("text-xl", creditCheckResult.score < 600 ? "text-destructive" : "text-primary")}>{creditCheckResult.score}</strong></p>
