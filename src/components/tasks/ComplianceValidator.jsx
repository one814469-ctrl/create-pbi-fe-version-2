...
const ComplianceValidator = ({ displayMessage }) => {
  ...
  if (!isAuthenticated || (user?.role !== 'officer' && user?.role !== 'underwriter' && user?.role !== 'manager')) {
    displayMessage('destructive', 'Access Denied', 'You need appropriate permissions to simulate compliance checks.');
    return (
      ...
      <CardContent className="space-y-6">
        ...
        {complianceStatus && (
          <div className="space-y-2">
            ...
            <p className={cn("text-sm", complianceStatus.compliant ? "text-muted-foreground" : "text-destructive")}>
