...
import NotificationSimulator from '../components/mocks/NotificationSimulator'; // For internal reminders
...
const InternalDashboardPage = ({ epics }) => {
  ...
  // Mock application data for dashboard
      ...
      <div className="dashboard-grid">
        <Card>
          ...
          <div className="table-container">
            ...
            <p className="mt-4"><em>Acceptance Criteria: {getSearchTask?.acceptance_criteria[0]} (Mocked)</em></p>
          </div>
        </Card>
        ...
        {selectedApplication && (
          <Card>
            ...
            {/* Display Credit Score to Loan Officers */}
            {getCreditScoreTask && (
                ...
                {selectedApplication.creditScore ? (
                  <div className="mock-api-response">
                    ...
                    {selectedApplication.creditScore < 600 && (
                      <p className="error-message"><em>Edge Case: "Given credit score is below the minimum threshold, When I view the report, Then the application is flagged for rejection and annotated" (Mocked)</em></p>
                    )}
                    <p className="success-message"><em>Acceptance Criteria: {getCreditScoreTask.acceptance_criteria[0]} (Mocked)</em></p>
                  </div>
                ) : (
            ...
            {/* Document Verification & OCR Mock */}
            {automatedDocVerificationEpic?.userStories.map(story => (
              ...
              <div key={story.title} className="mt-4 p-4 border-dashed">
                ...
                <OcrExtractionMock onUpdateApplication={(val) => handleUpdateApplication(selectedApplication.id, val)} />
                {selectedApplication.ocrData && (
            ...
            {/* Credit Check Integration Mock */}
            {creditCheckEpic?.userStories.map(story => (
              story.title === "Credit Bureau API Integration" && story.tasks[0] &&
              <div key={story.title} className="mt-4 p-4 border-dashed">
                <h4>{story.tasks[0].title}</h4>
                <CreditCheckMock onUpdateApplication={(val) => handleUpdateApplication(selectedApplication.id, val)} />
              </div>
            ))}

            {/* Approval Workflow Automation Mock */}
            {approvalWorkflowEpic?.userStories.map(story => (
              story.title === "Automated Decision Engine" && story.tasks[0] &&
              <div key={story.title} className="mt-4 p-4 border-dashed">
                <h4>{story.tasks[0].title}</h4>
                <DecisionEngineMock onUpdateApplication={(val) => handleUpdateApplication(selectedApplication.id, val)} />
              </div>
            ))}
            {approvalWorkflowEpic?.userStories.map(story => (
              ...
              <div key={story.title} className="mt-4 p-4 border-dashed">
                <h4>{story.tasks[0].title}</h4>
                <ComplianceMock onUpdateApplication={(val) => handleUpdateApplication(selectedApplication.id, val)} />
              </div>
            ))}
            ...
            {getAuditTrailTask && (
              <div className="mt-4 p-4 border-dashed">
                ...
                <div className="mock-api-response">
                  ...
                  <p className="mt-2"><em>Acceptance Criteria: {getAuditTrailTask.acceptance_criteria[0]} (Mocked)</em></p>
                </div>
              </div>
            )}
          </Card>
        ...
        {getInternalReminderTask && (
          <Card>
            ...
            <NotificationSimulator notificationType="internal_reminder" onUpdateApplication={(val) => handleUpdateApplication('internal', val)} />
            {/* Display mock reminders if any */}
            {mockApplications.find(app => app.id === 'internal')?.internalReminders && (
              <div className="mock-api-response mt-2">
                <h5>Recent Reminders:</h5>
                <ul className="notification-list">
                  {mockApplications.find(app => app.id === 'internal').internalReminders.map((rem, idx) => (
